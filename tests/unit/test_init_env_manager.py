...
class TestMergeEnvFile:
    """Tests for merge_env_file function."""

    def test_merge_adds_new_vars(self, tmp_path):
        """Test that merge adds new variables to existing .env."""
        env_file = tmp_path / ".ploston" / ".env"
        env_file.parent.mkdir(parents=True)
        env_file.write_text(
            "# Generated by: ploston init --import\n"
            "PLOSTON_RUNNER_TOKEN=plr_existing\n"
            "GITHUB_TOKEN=ghp_abc\n"
        )

        merge_env_file(
            {"NEW_VAR": "new_value", "ANOTHER": "val"},
            section="Test",
            env_file_path=env_file,
        )
        ...
        assert result["PLOSTON_RUNNER_TOKEN"] == "plr_existing"
        assert result["GITHUB_TOKEN"] == "ghp_abc"
        # New vars added
        assert result["NEW_VAR"] == "new_value"
        assert result["ANOTHER"] == "val"

    def test_merge_does_not_overwrite_existing(self, tmp_path):
        """Test that merge does not overwrite existing variables."""
        env_file = tmp_path / ".ploston" / ".env"
        env_file.parent.mkdir(parents=True)
        env_file.write_text("PLOSTON_RUNNER_TOKEN=plr_existing\nMY_VAR=original\n")

        merge_env_file(
            {"MY_VAR": "attempted_overwrite"},
            section="Test",
            env_file_path=env_file,
        )

        result = load_env_file(env_file)
        assert result["MY_VAR"] == "original"

    def test_merge_observability_env_vars(self, tmp_path):
        """Test merging OTEL env vars for observability (DEC-149).

        Simulates what bootstrap does when with_observability=True:
        injects OTEL vars into ~/.ploston/.env so the local runner
        forwards logs/traces to the collector.
        """
        env_file = tmp_path / ".ploston" / ".env"
        env_file.parent.mkdir(parents=True)
        env_file.write_text(
            "# Generated by: ploston init --import\n"
            "PLOSTON_RUNNER_TOKEN=plr_test\n"
        ...
        result = load_env_file(env_file)
        assert result["OTEL_EXPORTER_OTLP_ENDPOINT"] == "http://localhost:4327"
        assert result["OTEL_EXPORTER_OTLP_INSECURE"] == "true"
        assert result["PLOSTON_LOGS_ENABLED"] == "true"
        assert result["PLOSTON_TRACES_ENABLED"] == "true"
        # Existing secrets preserved
        assert result["GITHUB_PERSONAL_ACCESS_TOKEN"] == "ghp_abc"
        assert result["PLOSTON_RUNNER_TOKEN"] == "plr_test"

    def test_merge_creates_file_if_missing(self, tmp_path):
        """Test that merge creates .env if it doesn't exist yet."""
        env_file = tmp_path / ".ploston" / ".env"
        ...
        assert env_file.exists()
        result = load_env_file(env_file)
        assert result["SOME_VAR"] == "some_value"
