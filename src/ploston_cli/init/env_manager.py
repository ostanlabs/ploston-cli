...
# Owner-only modes for the secret file + its parent directory.
_ENV_FILE_MODE = 0o600
_ENV_DIR_MODE = 0o700


def _ensure_secret_dir(env_file: Path) -> None:
    """Create the .env parent directory with owner-only (0o700) perms."""
    parent = env_file.parent
    parent.mkdir(parents=True, exist_ok=True)
    try:
        os.chmod(parent, _ENV_DIR_MODE)
    except OSError:
...
def write_env_file_with_sections(
    ...
    atomic_write_text(env_file, "\n".join(lines), mode=_ENV_FILE_MODE)
    return env_file
